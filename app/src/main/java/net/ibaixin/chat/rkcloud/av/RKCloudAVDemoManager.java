...
public class RKCloudAVDemoManager implements RKCloudAVNewCallCallBack {
    ...
    private boolean mIsCallActivityShow = true;// 音视频通话的窗口是否为可见状态 true：可见
                                               // false:不可见
    private boolean mFloatDefaultShowSelf = true;// 悬浮窗中是否默认显示用户自己的图像
                                                 // true:表示默认显示自己的图像
    ...
    private boolean mSwitchCameraStatus = false;// 切换摄像头按钮的按下状态 true:表示为按下状态
                                                // false:正常状态
    ...
    public void onMissedCall(String callerAccount, boolean isVideo, long callTime) {
        ...
        //
        // // 同步用户信息
        // RKCloudAVContactManager.syncContactInfo(callerAccount);
    };
                    ...
                    case RKCloudAVErrorCode.AV_NO_REASON:
                        if (null != mCurrCallInfo && mCurrCallInfo.callAnswerTime > 0) {
                            long duration = (long) Math
                                    .ceil((System.currentTimeMillis() - mCurrCallInfo.callAnswerTime) / 1000);
                            mmsContent = mContext.getString(R.string.rkcloud_av_msg_callduration,
                                    RKCloudAVUtils.secondConvertToTime(duration));
                        }
                        break;
                    }
                    if (null != mmsContent) {
     ...
     * @param showName
     * @param isVideo
     *            是否为视频通话 true:视频通话 false:语音通话
     */
    public void dial(final Context context, final String calleeAccount, final String showName, final boolean isVideo) {
        // 条件判断
        ...
        RKCloudAV.rkCloudAVManager.dial(calleeAccount, isVideo, new RKCloudAVStateCallBack() {
            ...
            public void onStateCallBack(int state, int stateReason) {
                ...
                if (RKCloudAVCallState.AV_CALL_STATE_PREPARING == state) {
                    ...
                    intent.putExtra(RKCloudAVDemoActivity.INTENT_KEY_SHOWNAME, showName);
                    ...
                    case RKCloudAVErrorCode.AV_NO_REASON:
                        if (null != mCurrCallInfo && mCurrCallInfo.callAnswerTime > 0) {
                            long duration = (long) Math
                                    .ceil((System.currentTimeMillis() - mCurrCallInfo.callAnswerTime) / 1000);
                            mmsContent = context.getString(R.string.rkcloud_av_msg_callduration,
                                    RKCloudAVUtils.secondConvertToTime(duration));
                        }
                        break;
                    }
     ...
     * 向即时通信中添加消息记录
     */
    // private void addCallMsg(LocalMessage msgObj, boolean isVideo, boolean
    // isNotify) {
    // msgObj.setExtension(
    // isVideo ? RKCloudChatConstants.FLAG_AVCALL_IS_VIDEO :
    // RKCloudChatConstants.FLAG_AVCALL_IS_AUDIO);
    // RKCloudChatMmsManager mmsManager =
    // RKCloudChatMmsManager.getInstance(mContext);
    // mmsManager.addLocalMsg(msgObj, SingleChat.class);
    // if (isNotify) {
    // // 向通知栏中发送消息
    // mmsManager.onReceivedMsg(mmsManager.queryChat(msgObj.getChatId()),
    // msgObj);
    // }
    // }
    ...
    public List<RKCloudAVCallLog> getAllCallLogs() {
        return null != RKCloudAV.rkCloudAVManager ? RKCloudAV.rkCloudAVManager.getAllCallLogs()
                : new ArrayList<RKCloudAVCallLog>();
    }
                    ...
                    if (null != mCurrCallInfo) {
                        if (mFloatWinClickToEnterCallUi) {
                            ...
                            if (mCurrCallInfo.isCaller) {
                                intent.putExtra(RKCloudAVDemoActivity.INTENT_KEY_OUTCALL_ACCOUNT,
                                        mCurrCallInfo.peerAccount);
                            } else {
                                intent.putExtra(RKCloudAVDemoActivity.INTENT_KEY_INCALL_ACCOUNT,
                                        mCurrCallInfo.peerAccount);
                            }
    ...
    public void showOutCallNotification(String calleeAccount, long time) {
        // update by dudejin
        // RKCloudAVContact contactObj =
        // RKCloudAVContactManager.getContactInfo(calleeAccount);
        // String showName = null != contactObj && null != contactObj.showName ?
        // contactObj.showName : calleeAccount;
        String showName = calleeAccount;
    ...
    public void showInCallNotification(String callerAccount, long time) {
        // update by dudejin
        // RKCloudAVContact contactObj =
        // RKCloudAVContactManager.getContactInfo(callerAccount);
        // String showName = null != contactObj && null != contactObj.showName ?
        // contactObj.showName : callerAccount;
        String showName = callerAccount;
