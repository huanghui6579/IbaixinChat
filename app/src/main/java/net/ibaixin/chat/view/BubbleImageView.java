...
public class BubbleImageView extends ImageView {
	...
	/**
	 * 箭头上下的偏移量，正数 向上 ，负数 向下
	 */
	private int mArrowOffset = 0;
	/**
	 * 箭头上下的偏移是否居中,如果为true，则bubble_arrowOffset属性无效
	 */
	private boolean mArrowOffsetCenter = true;
	/**
	 * 箭头是否在整个图片高度的中间，默认为false
	 */
	private boolean mArrowCenter = false;
	...
	private void initView(AttributeSet attrs) {
		if (attrs != null) {
			...
			mAngle = res.getDimensionPixelSize(R.dimen.chat_msg_img_arrow_radius);
			mArrowTop = res.getDimensionPixelSize(R.dimen.chat_msg_img_arrow_top);
			mArrowWidth = res.getDimensionPixelSize(R.dimen.chat_msg_img_arrow_width);
			mArrowHeight = res.getDimensionPixelSize(R.dimen.chat_msg_img_arrow_height);
			
			TypedArray a = getContext().obtainStyledAttributes(attrs,
					R.styleable.BubbleImageView);
			mArrowOffsetCenter = a.getBoolean(R.styleable.BubbleImageView_bubble_arrowCenterOffset, true);
			mArrowCenter = a.getBoolean(R.styleable.BubbleImageView_bubble_arrowCenter, false);
			...
			mArrowHeight = (int) a.getDimension(
					...
					mArrowHeight);
			if (mArrowOffsetCenter) {
				mArrowOffset = - mArrowHeight / 2;
			}
	...
	protected void onDraw(Canvas canvas) {
		...
		rect.set(getPaddingLeft(), getPaddingTop(), getRight()
				- getLeft() - getPaddingRight(), getBottom() - getTop()
				- getPaddingBottom());

		if (mArrowCenter && mBitmapHeight > 0) {
			mArrowTop = mBitmapHeight / 2;
		}
